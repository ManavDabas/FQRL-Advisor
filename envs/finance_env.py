...
class FinanceEnv(gym.Env):
    ...
    def __init__(self,
        ...
        # -----------------------------
        # Realistic Market Regime Simulation
        # -----------------------------

        # 18 months bull market (positive mean)
        bull_market = self._rng.normal(0.01, 0.02, 18)

        # 18 months bear market (negative mean, higher volatility)
        bear_market = self._rng.normal(-0.02, 0.04, 18)

        self.market_returns = np.concatenate([bull_market, bear_market]).astype(float)
        crash_month = 20  # month index

        if 0 <= crash_month < len(self.market_returns):
            self.market_returns[crash_month] = float(-0.30)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.month = 0
        self.market_step = 0
        self._rng.shuffle(self.market_returns)
    ...
    def step(self, action):
        market_return = self.market_returns[self.market_step]
        self.market_step += 1


        if self.market_step >= len(self.market_returns):
           self.market_step = len(self.market_returns) - 1
        ...
        self.invested *= (1 + 1.5*market_return)
